...
type Callback = (operand?: any) => unknown;
...
async function test(message: string, testFunc: Callback, checkFunc: Callback): Promise<void> {
	try {
		const result = await testFunc();
		if (checkFunc(result)) pass(message);
		else fail(`${message}\n\t(Bad result: "${result}")`);
	} catch (err: any) {
		fail(`${message}:\n\tCaused by Error:\n\t${err.toString()}`);
	}
...
(async () => {
	...
	await Promise.allSettled([
		...
		test(
			"Dispatcher Exec 1 Operand",
			async () => {
				const reactor = {
					"testCommand2": (op1: string) => `Hello ${op1}`
				};
				const args = {
		...
		test(
			"Dispatcher Exec 2 Operands",
			async () => {
				const reactor = {
					"testCommand3": (op1: string, op2: string) => `Hello ${op1} ${op2}`
				};
				const args = {
		...
		test(
			"Dispatcher Exec Missing Operands",
			async () => {
				const reactor = {
					"testCommand4": (op1: string) => `Hello ${op1}`
				};
				const args = {
		...
		test(
			"Dispatcher Exec Invalid Command",
			async () => {
				const reactor = {
