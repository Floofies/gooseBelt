...
async function cliReactor() {
	...
	const reactor = {
		...
		poll: async (pollrate: string) => {
			config.pollrate = Number(pollrate);
			await configFile.write(config);
		},
		add: async (nickname: string, host: string) => {
			config.devices[nickname] = host;
			await configFile.write(config);
		},
		remove: async (nickname: string) => {
	...
	const output = <string> await cli.exec(process.argv[2], process.argv[3], process.argv[4]) ?? "\n";
